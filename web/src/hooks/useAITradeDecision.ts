...
export const normalizeAITradeDecision = (stock: any): AITradeDecision => {
  ...
  return {
    ...
    expectedValue: parseNum(stock.expected_value),
    normalizedCurrentPrice: parseNum(stock.normalized_current_price) || parseNum(stock.current_price),
    underlyingPrice: parseNum(stock.underlying_price),
    priceStatus: stock.price_status
  };
};
