...
export default function LiveSignalCard({ stock, decision }: LiveSignalCardProps) {
  ...
  const current = decision.normalizedCurrentPrice || stock.last_price;
    ...
    >
      ...
      <Box sx={{ px: 2, pb: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            />
            {isTradeActive && stock.price_status && (
                <Typography variant="caption" sx={{ fontSize: '0.5rem', fontWeight: 900, color: stock.price_status === 'FRESH' ? '#10b981' : '#ef4444' }}>
                    • {stock.price_status}
                </Typography>
            )}
          </Stack>
      ...
      <Box sx={{ px: 2, py: 2, borderTop: '1px solid rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
        <Grid container spacing={2}>
           ...
           <Grid item xs={6} sx={{ textAlign: 'right' }}>
              <Typography variant="caption" sx={{ color: 'slategray', fontWeight: 800, fontSize: '0.55rem', display: 'block', mb: 0.5 }}>{isOptions ? 'OPTION PREMIUM' : isFutures ? 'FUTURES CURRENT' : 'CURRENT PRICE'}</Typography>
              <Typography variant="body2" sx={{ fontWeight: 900, fontFamily: 'JetBrains Mono', color: (current && entry) ? (isBuy ? (current >= entry ? '#10b981' : '#ef4444') : (current <= entry ? '#10b981' : '#ef4444')) : '#fff' }}>
                {current ? `₹${current.toLocaleString()}` : (decision.status === 'UNAVAILABLE' ? 'N/A' : 'DATA_MISSING')}
              </Typography>
           </Grid>
           <Grid item xs={6}>
      ...
      <Box sx={{ p: 2, flexGrow: 1 }}>
        ...
        <Stack direction="row" justifyContent="space-between" sx={{ mt: 2 }}>
           ...
           <Box sx={{ textAlign: 'center' }}>
              <Typography variant="caption" sx={{ color: 'slategray', fontSize: '0.55rem', fontWeight: 800, display: 'block' }}>EXPECTED VALUE</Typography>
              <Typography variant="body2" sx={{ fontWeight: 900, color: (decision.expectedValue || 0) > 0 ? '#10b981' : '#ef4444', fontSize: '0.75rem' }}>
                {decision.expectedValue !== undefined ? `₹${decision.expectedValue.toFixed(1)}` : '—'}
              </Typography>
           </Box>
           <Box sx={{ textAlign: 'right' }}>
