...
export default function HistoricalPatternMatch({ symbol }: { symbol: string }) {
  ...
  return (
    <Box sx={{ mb: 4 }}>
      <Typography variant="h6" fontWeight="bold" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Layers size={20} className="text-blue-500" /> Historical Pattern Match
      </Typography>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Grid container spacing={3}>
           {matches && matches.length > 0 ? matches.map((m, i) => (
