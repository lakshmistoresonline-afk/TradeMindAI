...
async def run_detection():
    ...
    try:
        ...
        # 1. Fetch Benchmarks (Resilient Fallback logic)
        nifty_df = await provider.fetch_history("NIFTY", period="3mo")
        if nifty_df.empty:
            print("   [!] NIFTY (mapped) empty, trying direct ^NSEI...")
            nifty_df = await provider.fetch_history("^NSEI", period="3mo")

        vix_df = await provider.fetch_history("INDIAVIX", period="3mo")
        if vix_df.empty:
            vix_df = await provider.fetch_history("^INDIAVIX", period="3mo")

        if nifty_df.empty:
            print("[!] Error: All Nifty fetch attempts failed.")
            return
