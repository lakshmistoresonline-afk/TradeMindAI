...
# 4. Dependency Check/Install (Smart Install)
Write-Host "[*] Verifying production dependencies..."
python -m pip install --upgrade pip

# Check for critical packages before full install to save time/prevent build errors if already satisfied
$CriticalPackages = @("sqlalchemy", "dotenv", "psycopg2", "pandas", "numpy", "sklearn")
$Missing = $false

foreach ($pkg in $CriticalPackages) {
    $import_name = $pkg
    if ($pkg -eq "dotenv") { $import_name = "dotenv" }
    elseif ($pkg -eq "psycopg2") { $import_name = "psycopg2" }
    elseif ($pkg -eq "sklearn") { $import_name = "sklearn" }
...
if (-not (Test-Path "backend/.env")) {
    if (Test-Path "backend/.env.example") {
        ...
        Write-Host "[!] ERROR: backend/.env missing and no .env.example found."
        exit 1
    }
}
