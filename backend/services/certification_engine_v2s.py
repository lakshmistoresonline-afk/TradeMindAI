...
class Phase2SCertificationEngine:
    """
    Workstream 20: Phase 2S Final F&O Live-Data Activation Gate.
    Enforces the boundary between engineering completeness and live data retrieval.
    """

    POLICY_VERSION = "2S.1.0"
    ...
    def evaluate_gate(name: str, status: str, mandatory: bool = True, reason: Optional[str] = None, evidence: Any = None) -> Dict[str, Any]:
        return {
            ...
            "reason": reason or ("PASS" if status == "PASS" else "Institutional activation gate reason required."),
    ...
    async def run_certification_audit(cls) -> Dict[str, Any]:
        with SessionLocal() as session:
            ...
            gates["provider_activation"] = cls.evaluate_gate(
                ...
                reason="All F&O providers unauthenticated. Credentials required for activation." if upstox_auth != "PASS" and dhan_auth != "PASS" else None
            )
            ...
            for s in fno_active:
                if s.underlying_price and s.derivative_current:
                    if abs(s.underlying_price - s.derivative_current) < 0.0001:
                        separation = "FAIL"
                        break
            gates["underlying_derivative_separation"] = cls.evaluate_gate("underlying_derivative_separation", separation, reason="Derivative premium contaminated with spot price." if separation == "FAIL" else None)
            ...
            final_status = "PHASE2S_FAIL"
            if overall_pass:
                final_status = "PHASE2S_PASS"
