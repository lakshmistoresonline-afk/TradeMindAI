...
class Phase2ICertificationEngine:
    """
    Workstream 33: Phase 2I Institutional Certification Engine.
    Enforces context-aware hard gates (Legacy vs Current).
    Ensures 100% field completeness for post-Ledger signals.
    """

    POLICY_VERSION = "2I.1.0"
    ...
    async def run_certification_audit(cls) -> Dict[str, Any]:
        with SessionLocal() as session:
            ...
            # 2. Current Signal Integrity (Workstream 23)
            current_signals = [s for s in active if s.timestamp >= cls.LEDGER_2_0_ENFORCEMENT_DATE]

            current_integrity = "PASS"
            if not current_signals:
                current_integrity = "NOT_APPLICABLE" # Or FAIL if we strictly require a new signal to exist
            else:
                for s in current_signals:
                    # Mandatory fields for current signals
                    mandatory_fields = [
                        s.prediction_id, s.provenance_id, s.feature_version, s.regime,
                        s.stop_price, s.risk_reward_ratio, s.asset_type,
                        s.decision_id, s.model_run_id, s.market_snapshot_id, s.signal_timestamp
                    ]
            ...
            if overall_pass:
                ...
                final_status = "PHASE2I_CONDITIONAL_PASS" if limitations else "PHASE2I_PASS"
