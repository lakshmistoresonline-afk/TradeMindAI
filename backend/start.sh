...
if [ "$ENVIRONMENT" = "production" ]; then
    echo "[*] AUDIT: Executing Alembic migrations via Python runner..."
    # Using python to run alembic to ensure path resolution is exact
    python -c "from alembic.config import Config; from alembic import command; alembic_cfg = Config('backend/alembic.ini'); command.upgrade(alembic_cfg, 'head')"
    if [ $? -ne 0 ]; then
