...
async def get_opportunities(limit: int = 10):
    ...
    Vision 2.0: Automatically triggers a 'Bootstrap Scan' if no deep results are found.
    """
    active = await container.ios_repo.get_active_opportunities(limit=limit)

    if not active:
        # Trigger on-the-fly bootstrap scan
        stocks = await container.repository.get_all_stocks(limit=100)
        bootstrap_opps = container.opportunity_engine.find_opportunities(stocks)

        # Save bootstrap results so they persist for the current session
        for opp in bootstrap_opps:
            await container.ios_repo.save_opportunity(opp)

        return bootstrap_opps[:limit]
